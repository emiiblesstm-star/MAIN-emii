...
const TradeUiClone = observer(() => {
  ...
  // UPDATED: Default ticks for higher/lower is 5, others are 1
  const [ticksByTradeType, setTicksByTradeType] = useState({
    digits: 1,
    evenodd: 1,
    risefall: 1,
    matchdiff: 1,
  ...
  // CORRECTED: Higher/Lower state variables from second code
  ...
  // PROFIT/LOSS TRACKING - FIXED: Remove duplicate totalProfitRef
  const [totalProfit, setTotalProfit] = useState<number>(0)
  const [totalLoss, setTotalLoss] = useState<number>(0)
  const [netPL, setNetPL] = useState<number>(0)
  const totalWinsRef = useRef<number>(0) // Changed from totalProfitRef to avoid duplicate
  const totalLossesRef = useRef<number>(0) // Changed from totalLossRef for consistency
  ...
  // Helper function to get current ticks based on active trade type
  const getCurrentTicks = () => ticksByTradeType[activeTradeKey]

  // Helper function to update ticks for current trade type
  const updateCurrentTicks = (value: number | null) => {
    setTicksByTradeType(prev => ({
      ...prev,
      [activeTradeKey]: value ?? 1
    }))
  }

  // Update profit/loss tracking
  const updateProfitLossTracking = (profit: number) => {
    if (profit > 0) {
      totalWinsRef.current += profit
      setTotalProfit(totalWinsRef.current)
    } else {
      totalLossesRef.current += Math.abs(profit)
      setTotalLoss(totalLossesRef.current)
    }
    
    const newNetPL = totalWinsRef.current - totalLossesRef.current
    setNetPL(newNetPL)
  }

  // SMART TRADER MARTINGALE LOGIC
  const applySmartTraderMartingaleLogic = (wasWin: boolean, profit: number) => {
    if (wasWin) {
      ...
      setStatus(`✅ Trade WIN! Profit: $${profit.toFixed(2)} | Reset to base stake: $${baseStakeRef.current}`)
    } else {
      ...
      if (recoveryModeEnabled && lossStreakRef.current >= 1 && 
        ...
        setStatus(`🔄 Recovery Mode ACTIVATED | Loss: $${Math.abs(profit).toFixed(2)} | Next stake: $${currentStakeRef.current} (x${martingaleStepRef.current})`)
      } else {
        setStatus(`🔴 Trade LOSS! Loss: $${Math.abs(profit).toFixed(2)} | Next stake: $${currentStakeRef.current} (x${martingaleStepRef.current})`)
      }
    }
  }
  ...
  const handleContractCompletion = (contractData: any) => {
    ...
    // Update profit/loss tracking
    updateProfitLossTracking(profit)
    ...
    if (!jetModeEnabledRef.current) {
      applySmartTraderMartingaleLogic(wasWin, profit)
    } else {
  ...
  // CORRECTED: Higher/Lower trade execution using CALL/PUT with barrier from second code
  ...
  // UPDATED: Purchase contract with CORRECT HIGHER/LOWER IMPLEMENTATION from second code
  const purchaseContract = async (
    ...
    // SPECIAL HANDLING FOR HIGHER/LOWER from second code
    ...
    const trade_option: any = {
      ...
      currency: account_currency,
      duration: Number(getCurrentTicks() || 0),
      duration_unit: "t",
      symbol: currentSym,
      ...additionalParams,
    }
  ...
  const startAutoTrading = async () => {
    ...
    // Reset profit/loss tracking when starting auto trading
    ...
    <div className="pro-trader">
      ...
      <div className="pro-trader__header">
        ...
        <div className="pro-trader__total-pl">
          <span className="pro-trader__total-pl-label">Net P/L:</span>
          <span className={`pro-trader__total-pl-value ${netPL >= 0 ? "pro-trader__total-pl-value--positive" : "pro-trader__total-pl-value--negative"}`}>
            ${netPL.toFixed(2)}
          </span>
        </div>
      </div>
        ...
        <div className="pro-trader__panel pro-trader__panel--config">
            ...
            <DynamicTradeCard
              ...
              statusText={
                activeTradeKey === "digits"
                  ? `Mode ${digitMode.toUpperCase()} • Prediction: ${predictionDigit !== null ? predictionDigit : "—"} • Current Stake: $${currentStakeRef.current} • Ticks: ${getCurrentTicks() ?? "—"} • Jet: ${jetModeEnabled ? "ON (FAST)" : "OFF (SAFE)"}`
                  : activeTradeKey === "evenodd"
                    ? `Parity ${parity.toUpperCase()} • Current Stake: $${currentStakeRef.current} • Ticks ${getCurrentTicks() ?? "—"} • Jet: ${jetModeEnabled ? "ON (FAST)" : "OFF (SAFE)"}`
                    : activeTradeKey === "risefall"
                      ? `Direction ${direction.toUpperCase()} • Current Stake: $${currentStakeRef.current} • Ticks ${getCurrentTicks() ?? "—"} • Jet: ${jetModeEnabled ? "ON (FAST)" : "OFF (SAFE)"}`
                      : activeTradeKey === "matchdiff"
                        ? `Type ${matchType.toUpperCase()} ${matchType === "matches" ? (matchDigit ?? "—") : "(any)"} • Current Stake: $${currentStakeRef.current} • Ticks ${getCurrentTicks() ?? "—"} • Jet: ${jetModeEnabled ? "ON (FAST)" : "OFF (SAFE)"}`
                        : `${higherLowerType.toUpperCase()} • Barrier: ${higherLowerBarrierType}${higherLowerBarrierValue ?? "—"} • Current Stake: $${currentStakeRef.current} • Ticks: ${getCurrentTicks() ?? "—"} • Jet: ${jetModeEnabled ? "ON (FAST)" : "OFF (SAFE)"}`
              }
            >
              {() => (
                <>
                    ...
                    <div className="form-grid">
                      ...
                      <div className="form-field">
                        ...
                        <input
                          type="number"
                          min={1}
                          value={getCurrentTicks() ?? ""}
                          onChange={(e) => updateCurrentTicks(e.target.value === "" ? null : Number(e.target.value))}
                          disabled={autoTrading}
                        />
                      </div>
                    </div>
                  ...
                  {/* CORRECTED: Higher/Lower form from second code */}
          ...
          {status && (
            <div className="pro-trader__status">
              ...
              <div className="pro-trader__status-item">
                <span className="pro-trader__status-label">Total Profit:</span>
                <span className="pro-trader__status-value pro-trader__status-value--positive">${totalProfit.toFixed(2)}</span>
              </div>
              <div className="pro-trader__status-item">
                <span className="pro-trader__status-label">Total Loss:</span>
                <span className="pro-trader__status-value pro-trader__status-value--negative">${totalLoss.toFixed(2)}</span>
              </div>
              <div className="pro-trader__status-item">
                <span className="pro-trader__status-label">Net P/L:</span>
                <span className={`pro-trader__status-value ${netPL >= 0 ? "pro-trader__status-value--positive" : "pro-trader__status-value--negative"}`}>
                  ${netPL.toFixed(2)}
                </span>
              </div>
